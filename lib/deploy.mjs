...
const PREPROD_INDEXER_URL = "https://indexer.midnight-preprod.midnight.global/api/v1/graphql";
...
async function deploy() {
  ...
  for (const file of requiredFiles) {
    try {
      ...
      console.error("   Run: compact compile -- pkgs/contract/src/patient_registry.compact managed/");
      process.exit(1);
    }
  }
  ...
  console.log(`   ✓ Witnesses: ${contractInfo.witnesses.map(w => w.name).join(", ")}`);
  ...
  try {
    ...
    console.log("   The contract is compiled and ready to deploy via the frontend.");
    console.log("   Connect Lace wallet at http://localhost:3000 to complete deployment.");
  ...
  // Step 5: Summary
  console.log("\n📋 Deployment Status:");
  console.log("   ✅ Contract source: pkgs/contract/src/patient_registry.compact");
